...
module.exports = class ScoopOperation extends Operation {
    ...
    supportRoomCallback(room) {
        ...
        //console.log(JSON.stringify(scoopers,0,2))
        if(scoopers.length < this.memory.scooperCount) {
            roomai.spawn(spawnHelper.bestAvailableParts(room, scooper.configs(1000)), { role: scooper.name, home: room.name, storageRoom: this.memory.supportRoom, target: this.memory.targetRoom, operation: this.id });
        }
    }
